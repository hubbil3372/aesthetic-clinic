...
class Ion_auth_model extends CI_Model
{
	...
	public function get_user_by_activation_code($user_code)
	{
		...
		if ($token) {
			...
			$user = $this->where('pengPemilihAktif', $token->selector)->users()->row();

			if ($user) {
				// Check the hash against the validator
				if ($this->verify_password($token->validator, $user->pengKodeAktivasi)) {
					return $user;
				}
			}
		}
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code === FALSE || ($user && $user->pengId === $id)) {
			$data = [
				'pengPemilihAktif' => NULL,
				'pengKodeAktivasi' => NULL,
				'pengAktif'        => 1,
				'pengStatus'       => 1
			];

			$this->trigger_events('extra_where');
			$this->db->update($this->tables['users'], $data, ['pengId' => $id]);
	...
	public function deactivate($id = NULL)
	{
		...
		$data = [
			'pengPemilihAktif' => $token->selector,
			'pengKodeAktivasi' => $token->validator_hashed,
			'pengAktif'        => 0
		];
	...
	public function clear_forgotten_password_code($identity)
	{
		...
		$data = [
			'pengPemilihLupaSandi' => NULL,
			'pengKodeLupaSandi' => NULL,
			'pengWaktuLupaSandi' => NULL
		];
	...
	public function change_password($identity, $old, $new)
	{
		...
		$query = $this->db->select('pengId, pengPass')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('pengId', 'desc')
		...
		if ($this->verify_password($old, $user->pengPass, $identity)) {
	 ...
	 * @author Mathew
	 */
	public function edit_password($identity, $new)
	{
		$this->trigger_events('pre_change_password');
	...
	public function forgotten_password($identity)
	{
		...
		$update = [
			'pengPemilihLupaSandi' => $token->selector,
			'pengKodeLupaSandi' => $token->validator_hashed,
			'pengWaktuLupaSandi' => time()
		];
	...
	public function get_user_by_forgotten_password_code($user_code)
	{
		...
		if ($token) {
			// Retrieve the user according to this selector
			$user = $this->where('pengPemilihLupaSandi', $token->selector)->users()->row();

			if ($user) {
				// Check the hash against the validator
				if ($this->verify_password($token->validator, $user->pengKodeLupaSandi)) {
					return $user;
				}
			}
		}
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$query = $this->db->get_where($this->tables['groups'], ['grupNama' => $this->config->item('default_group', 'ion_auth')], 1)->row();

		if (!isset($query->grupId) && empty($groups)) {
		...
		$data = [
			...
			'pengPass' => $password,
			'pengEmail' => $email,
			'pengFoto' => 'default.jpg',
			'pengIpAddress' => $ip_address,
			'pengDibuatPada' => date("Y-m-d H:i:s"),
			'pengAktif' => ($manual_activation === FALSE || $email_activation === FALSE ? 1 : 0),
			'pengStatus' => ($manual_activation === FALSE || $email_activation === FALSE ? 1 : 0),
		];
		...
		$row = $this->db->select("*")->limit(1)->order_by('pengDibuatPada', "DESC")->get('pengguna')->row();
		$id = $row->pengId;
		...
		if (isset($default_group->grupId) && empty($groups)) {
			$groups[] = $default_group->grupId;
		}
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$query = $this->db->select('*')
			...
			->join('pengguna_grup', "pengguna_grup.pgrupPengId = {$this->tables['users']}.pengId")
			->join('grup', 'grup.grupId = pengguna_grup.pgrupGrupId')
			->where($this->identity_column, $identity)
			->limit(1)
			->order_by('pengId', 'desc')
			->get();
		...
		if ($this->is_max_login_attempts_exceeded($identity)) {
			...
			$this->session->set_flashdata('error', 'Sementara Terkunci. Coba Beberapa Saat Lagi!');
		...
		if ($query->num_rows() === 1) {
			...
			if ($this->verify_password($password, $user->pengPass, $identity)) {
				if ($user->pengAktif == 0) {
					$this->trigger_events('post_login_unsuccessful');
				...
				if ($user->pengStatus == null) {
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error('login_unsuccessful_not_status');
					$this->session->set_flashdata('error', 'Akun Tidak Aktif!');
				...
				$this->update_last_login($user->pengId);
				...
				// Rehash if needed
				$this->rehash_password_if_needed($user->pengPass, $identity, $password);
		...
		$this->trigger_events('post_login_unsuccessful');
		$this->set_error('login_unsuccessful');
		$this->session->set_flashdata('error', 'Kombinasi kata sandi salah!');
	...
	public function recheck_session()
	{
		...
		if ($recheck !== 0) {
			$pengLoginTerakhir = $this->session->userdata('last_check');
			if ($pengLoginTerakhir + $recheck < time()) {
	...
	public function users($groups = NULL)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			$this->db->select([
				$this->tables['users'] . '.*',
				$this->tables['users'] . '.pengId as id',
				$this->tables['users'] . '.pengId as user_id'
			]);
		}
		...
		if (isset($groups)) {
			...
			if (isset($groups) && !empty($groups)) {
				...
				$this->db->join(
					...
					$this->tables['users_groups'] . '.' . $this->join['users'] . '=' . $this->tables['users'] . '.pengId',
					'inner'
				);
			}
			...
			if (!empty($group_names)) {
				$this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.grupId', 'inner');
				$this->db->where_in($this->tables['groups'] . '.grupNama', $group_names);
			}
			if (!empty($group_ids)) {
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('id');

		$this->limit(1);
		$this->order_by($this->tables['users'] . '.pengId', 'desc');
		$this->where($this->tables['users'] . '.pengId', $id);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.grupNama, ' . $this->tables['groups'] . '.grupDeskripsi')
			->where($this->tables['users_groups'] . '.' . $this->join['users'], $id)
			->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . '=' . $this->tables['groups'] . '.grupId')
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE)
	{
		...
		if (isset($this->_cache_user_in_group[$id])) {
			...
			foreach ($users_groups as $group) {
				$groups_array[$group->id] = $group->grupNama;
			}
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				...
				$this->db->delete(
					$this->tables['users_groups'],
					[$this->join['groups'] => $group_id, $this->join['users'] => $user_id]
				);
		...
		// otherwise remove user from all groups
			...
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['users_groups'], [$this->join['users'] => $user_id])) {
	...
	public function group($id = NULL)
	{
		...
		if (isset($id)) {
			$this->where($this->tables['groups'] . '.grupId', $id);
		}

		$this->limit(1);
		$this->order_by('grupId', 'desc');
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->identity_column, $data) || array_key_exists('pengPass', $data) || array_key_exists('email', $data)) {
			if (array_key_exists('pengPass', $data)) {
				if (!empty($data['pengPass'])) {
					$user = $this->user($id)->row();
					$data['pengPass'] = $this->hash_password($data['pengPass'], $user->{$this->identity_column});
					if ($data['pengPass'] === FALSE) {
						$this->db->trans_rollback();
						$this->trigger_events(['post_update_user', 'post_update_user_unsuccessful']);
						$this->set_error('update_unsuccessful');
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], ['pengLoginTerakhir' => time()], ['pengId' => $id]);
	...
	public function set_session($user)
	{
		...
		$session_data = [
			...
			'id'                       => $user->pengId, //everyone likes to overwrite id so we'll use user_id
			'nama'		                 => $user->pengNama, //everyone likes to overwrite id so we'll use user_id
			'foto'		                 => $user->pengFoto, //everyone likes to overwrite id so we'll use user_id
			'grupId'                   => $user->grupId,
			'last_check'               => time(),
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, remember_code, pengLoginTerakhir')
